...
export const GetProductData = () => {
  const res = client.fetch(`*[_type == "product" && "latest" in tags]{
   name,
   price,
      ...
      <div className="flex-col justify-center items-center gap-2 md:gap-5 flex">
        ...
        <div className="subContainer grid grid-cols-1 sm:grid-cols-2  clg:grid-cols-3 gap-5 ">
          {product.map((item: Product, index) => (
            <div
              key={index}
              className="  w-[280px] sm:w-[300px] llm:w-[365px] group  h-[306px] shadow-md hover:shadow-xl transition-shadow duration-300"
            >
                ...
                <Link href={`/section4/${item._id}`}>
                  ...
                  <div className="top-[190px] left-[23px] absolute hidden group-hover:flex">
                    {" "}
