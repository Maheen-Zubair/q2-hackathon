...
const GetProductData = () => {
  const res = client.fetch(`*[_type == "product" && "trending" in tags]{
      _id,
     name,
     price,
    ...
    <div className="flex flex-col text-[#151875] pt-24 pb-24 items-center  gap-[10px] md:gap-[20px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 justify-center items-center">
        {product.map((item: Product, index) => (
          <div
            key={index}
            className=" blog w-[270px] h-[350px]  flex-col flex items-center pt-3 shadow-md hover:shadow-xl transition-shadow duration-300"
          >
                      <Link href={`/section5b/${item._id}`}>
